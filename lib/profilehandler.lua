...
local function apply_root_color_overrides()
	local ok, root_colors = pcall(require, 'colors')
	if not ok or type(root_colors) ~= 'table' then
		return
	end

	if type(gProfileColor) ~= 'table' then
		gProfileColor = T{}
	end

	for k, v in pairs(root_colors) do
		gProfileColor[k] = v
	end
end
...
status.Init = function()
	if (AshitaCore:GetMemoryManager():GetParty():GetMemberIsActive(0) == 1) then
		...
		gStatus.SettingsFolder = ('%sconfig\\addons\\combatlog\\%s_%u\\'):fmt(AshitaCore:GetInstallPath(), gStatus.PlayerName, gStatus.PlayerId);
...
status.AutoLoadProfile = function()
	...
	if (not ashita.fs.exists(defaultSettingsFile)) then
		...
		print(chat.header('CombatLog') .. chat.message('Created config file: ') .. chat.color1(2, 'config.lua'));
	...
	if (not ashita.fs.exists(jobFiltersFile)) then
		if (not ashita.fs.exists(defaultFiltersFile)) then
			...
			print(chat.header('CombatLog') .. chat.message('Created filters profile: ') .. chat.color1(2, 'default_filters.lua'));
	...
	if (not ashita.fs.exists(defaultColorsFile)) then
		...
		print(chat.header('CombatLog') .. chat.message('Created color profile: ') .. chat.color1(2, 'chat_colors.lua'));
...
status.LoadProfile = function(profilePath, profileType)
	...
	if (profileType == 'config') then
		if not success then
			...
			print(chat.header('CombatLog') .. chat.error('Failed to load configuration file: ') .. chat.color1(2, shortFileName)..chat.error('\nSaving will be disabled.'));
			print(chat.header('CombatLog') .. chat.error(loadError));
			static_config = true
			return;
		end
		gProfileSettings = success();
		if (gProfileSettings ~= nil) then
			print(chat.header('CombatLog') .. chat.message('Loaded configuration file: ') .. chat.color1(2, shortFileName));
		end
	end
	...
	if (profileType == 'filters') then
		if not success then
			local defaultFiltersFile = gStatus.SettingsFolder .. 'default_filters.lua';
			print(chat.header('CombatLog') .. chat.error('Failed to load filters profile: ') .. chat.color1(2, shortFileName) .. chat.error(' loading defaults: ' .. chat.color1(2, 'default_filters.lua')));
			print(chat.header('CombatLog') .. chat.error(loadError));
			...
			if not default_success then
				...
				print(chat.header('CombatLog') .. chat.error('Failed to load filters profile: ') .. chat.color1(2, 'default_filters.lua')..chat.error('\nSaving will be disabled.'));
				print(chat.header('CombatLog') .. chat.error(default_loadError));
				static_config = true
				return
			end
		...
		if (gProfileFilter ~= nil) then
			print(chat.header('CombatLog') .. chat.message('Loaded filters profile: ') .. chat.color1(2, shortFileName));
	...
	if (profileType == 'colors') then
		if not success then
			...
			apply_root_color_overrides()
			print(chat.header('CombatLog') .. chat.error('Failed to load colors profile: ') .. chat.color1(2, shortFileName)..chat.error('\nSaving will be disabled.'));
			print(chat.header('CombatLog') .. chat.error(loadError));
			static_config = true
			return;
		end
		...
		if (gProfileColor ~= nil) then
			print(chat.header('CombatLog') .. chat.message('Loaded colors profile: ') .. chat.color1(2, shortFileName));
		end
	end
end
