...
-- Resource lookup proxies: lazily forward index access to Ashita's ResourceManager
-- and cache the result on first use. This avoids pre-loading the entire item/spell/
-- skill databases into Lua tables at startup (previously cost ~20 MB and thousands
-- of redundant API calls iterating up to 100,000 items).
local function make_resource_proxy(lookup_fn)
    return setmetatable({}, {__index = function(t, id)
        local v = lookup_fn(id)
        if v ~= nil then rawset(t, id, v) end
        return v
    end})
end

-- Weapon skills: game IDs 1..0x1FF → table index == game ID
get_weapon_skill = make_resource_proxy(function(id)
    return AshitaCore:GetResourceManager():GetAbilityById(id)
end)

-- Job abilities: game IDs 0x201..0x600 → table index = game ID - 0x200
get_job_ability = make_resource_proxy(function(id)
    return AshitaCore:GetResourceManager():GetAbilityById(id + 0x200)
end)

-- Monster skills: game IDs 0x601+ → table index = game ID - 0x600
get_mon_skill = make_resource_proxy(function(id)
    return AshitaCore:GetResourceManager():GetAbilityById(id + 0x600)
end)

-- Monster abilities: stored at index 0x101+k for string index k+1
-- → string index = id - 0x100
get_mon_ability = make_resource_proxy(function(id)
    local string_idx = id - 0x100
    local en = AshitaCore:GetResourceManager():GetString('monsters.abilities', string_idx, 2)
    local jp = AshitaCore:GetResourceManager():GetString('monsters.abilities', string_idx, 1)
    if not en then return nil end
    return {Name = {en, jp}}
end)

-- Spells: game IDs sequential from 1 → table index == game ID
get_spell = make_resource_proxy(function(id)
    return AshitaCore:GetResourceManager():GetSpellById(id)
end)

-- Items: game IDs sequential from 1 → table index == game ID
get_item = make_resource_proxy(function(id)
    return AshitaCore:GetResourceManager():GetItemById(id)
end)

-- Kept as no-ops for API compatibility; proxies above are initialized at load time.
local PopulateSkills = function() end
local PopulateSpells = function() end
local PopulateItems = function() end
...
local SendDelayedMessage = function (color, msg)
    ...
    gConfig.add_line(color, message)
    item_quantity.id = 0
    item_quantity.count = ''
...
local Error = function(text)
    print(chat.header('CombatLog') .. chat.error(text));
end
