...
texthandlers.HandleIncomingText = function(e)
	local mode_settings = gProfileSettings and gProfileSettings.mode
	if not mode_settings then
		return
	end

	local redcol = e.mode%256
	
	if (redcol == 121 and mode_settings.cancelmultimsg) then
	...
	elseif (redcol == 123 and mode_settings.cancelmultimsg) then
