...
local function ensure_profile_globals()
    if gProfileSettings == nil then
        gProfileSettings = static_settings or gDefaultSettings or T{
            lang = T{ object = 1, internal = 2, msg_text = 'en' },
            mode = T{},
            text = T{},
        }
    end
    ...
    if gProfileColor == nil then
        gProfileColor = static_colors or T{}
    end

    if gProfileFilter == nil then
        gProfileFilter = static_filters or T{}
    end
end
...
------- ACTION MESSAGE -------
    elseif e.id == 0x29 then
        ...
        elseif passed_messages:contains(am.message_id) then
            ...
            gConfig.add_line(res_actmsg[am.message_id]['color'], outstr)
            am.message_id = false
        end
...
------------ SYNTHESIS ANIMATION --------------
    elseif e.id == 0x030 and gProfileSettings.mode.crafting then
        ...
        if Self.ServerId == struct.unpack('I', e.data, 5) or target_id == struct.unpack('I', e.data, 5) then
            ...
            if result == 0 then
                gConfig.add_line(8, ' ------------- NQ Synthesis ('..crafter_name..') -------------')
            elseif result == 1 then
                gConfig.add_line(8, ' ---------------- Break ('..crafter_name..') -----------------')
            elseif result == 2 then
                gConfig.add_line(8, ' ------------- HQ Synthesis ('..crafter_name..') -------------')
            else
                gConfig.add_line(8, 'Craftmod: Unhandled result '..tostring(result))
            end
        end
    elseif e.id == 0x06F and gProfileSettings.mode.crafting then
        if e.data:byte(5) == 0 or e.data:byte(5) == 12 then
            ...
            if result == 1 then
                gConfig.add_line(8, ' -------------- HQ Tier 1! --------------')
            elseif result == 2 then
                gConfig.add_line(8, ' -------------- HQ Tier 2! --------------')
            elseif result == 3 then
                gConfig.add_line(8, ' -------------- HQ Tier 3! --------------')
            end
        end
...
packethandlers.multi_packet = function(...)
    ...
    gConfig.add_line(res_actmsg[multi_msg[ind]].color, outstr)
    multi_targs[ind] = nil
    multi_msg[ind] = nil
    multi_actor[ind] = nil
end
